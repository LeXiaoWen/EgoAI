...
function shouldRefreshProxyToken(status: number, provider?: string): boolean {
  ...
  return status === 403 && provider !== ProviderName.EgoaiServer;
}

function isTemporaryEgoAIAuthRefreshFailure(
  provider: string | undefined,
  result: AuthTokenRefreshResult,
): boolean {
  return provider === ProviderName.EgoaiServer
      ...
      if (
        ...
        if (refresher) {
          ...
          try {
            ...
            } else if (isTemporaryEgoAIAuthRefreshFailure(
              upstreamConfig.provider,
              refreshResult,
            )) {
              writeJSON(
...
export const __openAICompatProxyTestUtils = {
  ...
  isTemporaryEgoAIAuthRefreshFailure,
  isGeminiProvider,
