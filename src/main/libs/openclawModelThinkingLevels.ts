...
const EGOAI_SERVER_MODEL_PREFIX = `${OpenClawProviderId.EgoaiServer}/`;
...
export const resolveOpenClawThinkingLevelForModel = (
  ...
  if (
    ...
    || !normalizedModelRef.startsWith(EGOAI_SERVER_MODEL_PREFIX)
  ) {
    return productLevel;
  }

  const modelId = normalizedModelRef.slice(EGOAI_SERVER_MODEL_PREFIX.length);
