...
describe('openclawCronLegacyMigration', () => {
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-openclaw-cron-migration-'));
  ...
  test('runs official doctor fix when jobs.json exists', async () => {
    ...
    expect(options.env.OPENCLAW_CONFIG_PATH).toBe(path.join(stateDir, '.egoai-cron-doctor-openclaw.json'));
