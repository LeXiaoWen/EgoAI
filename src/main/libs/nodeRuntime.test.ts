...
beforeEach(() => {
  mockElectronState.appPath = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-node-runtime-'));
  mockElectronState.isPackaged = false;
  mockElectronState.userData = path.join(os.tmpdir(), 'egoai-user-data');
});
...
describe('selectSpawnableNodeCandidate', () => {
  test('skips EgoAI bash and cmd shims on Windows and returns node.exe', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\EgoAI\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\EgoAI\\cowork\\bin\\node.cmd',
      'C:\\Program Files\\nodejs\\node.exe',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\EgoAI')).toBe(
      'C:\\Program Files\\nodejs\\node.exe',
    );
  });
  ...
  test('returns null on Windows when only non-native shims are available', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\EgoAI\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\EgoAI\\cowork\\bin\\node.cmd',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\EgoAI')).toBeNull();
  });
...
describe('resolveNodeRuntimeForSpawn', () => {
  test('uses real node.exe on Windows when present after EgoAI shims', () => {
    setPlatform('win32');
