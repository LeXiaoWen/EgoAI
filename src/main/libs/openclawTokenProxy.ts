...
import { isEgoAIQuotaExhaustedError } from '../../common/coworkErrorClassify';
import {
  AuthRefreshOutcome,
  AuthRefreshReason,
...
function shouldRefreshEgoAIToken(status: number): boolean {
  return status === 401;
}
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  try {
    ...
    if (shouldRefreshEgoAIToken(result.status) && tokenRefresher) {
...
function extractQuotaErrorFromProxyErrorPayload(
  ...
  if (proxyError) {
    ...
    return isEgoAIQuotaExhaustedError(searchable) ? proxyError : null;
  }
  return event === 'error' && isEgoAIQuotaExhaustedError(payload)
    ? { message: payload }
    : null;
}
...
function buildUpstreamRequestHeaders(
  ...
  const headers: Record<string, string> = {
    ...
    [EGOAI_CLIENT_CAPABILITIES_HEADER]: EGOAI_CLIENT_CAPABILITIES,
    [EGOAI_CLIENT_VERSION_HEADER]: clientVersion,
  };
...
function pipeResponse(
  ...
  if (result.isStream) {
    ...
    scanProxyBodyForQuotaError(result.body, Date.now());
    res.end(result.body);
  } else {
...
function pipeStreamingResponseWithQuotaScan(
  ...
  if (Buffer.isBuffer(body)) {
    scanProxyBodyForQuotaError(body, Date.now());
    res.end(body);
    return;
  }
  ...
  const scanState = createProxySSEStreamScanState(Date.now());
