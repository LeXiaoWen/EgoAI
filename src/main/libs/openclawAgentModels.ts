...
export function resolveServerModelRefForRun(options: {
  ...
  if (explicitTarget) {
    if (explicitTarget.providerId === OpenClawProviderId.EgoaiServer) {
      return {
  ...
  const matchingProviders = Object.entries(options.availableProviders)
    ...
    .map(([providerId]) => providerId);
  const serverMatched = matchingProviders.includes(OpenClawProviderId.EgoaiServer);

  if (serverMatched && matchingProviders.length === 1) {
    return {
      status: ServerModelRefResolutionStatus.Server,
      modelId: modelRef,
      primaryModel: `${OpenClawProviderId.EgoaiServer}/${modelRef}`,
    };
  }
  if (serverMatched) {
    return {
