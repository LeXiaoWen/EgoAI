// Renders EgoAI provider configuration into the dsh settings file
...
// this marker: without it a EgoAI-managed provider is indistinguishable
// from one the user added inside dsh.
export const DSH_MANAGED_LABEL_PREFIX = 'EgoAI · ';
...
export const DSH_PLAN_ANTHROPIC_ROUTE_ID = 'egoai-plan-anthropic';
...
export function mapApiFormatToDshProtocol(apiFormat: string | undefined): DshProviderRoute['api'] | null {
  ...
  // EgoAI's OpenAI-format providers are overwhelmingly third-party
...
export function renderDshManagedSettings(
  ...
  for (const [providerId, config] of Object.entries(providers)) {
    ...
    routes[routeId] = {
      ...
      // ("deepseek"), and mark the entry as EgoAI-managed.
      displayName:
