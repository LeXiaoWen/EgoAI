...
describe('McpBridgeServer AskUser session attribution', () => {
  test('passes sessionKey from HTTP AskUser callback requests', async () => {
    ...
    try {
      ...
      const response = await fetch(url!, {
        ...
        body: JSON.stringify({
          sessionKey: 'agent:main:egoai:session-a',
      ...
      expect(received[0].sessionKey).toBe('agent:main:egoai:session-a');
    } finally {
      await server.stop();
    }
  });
  ...
  test('passes sessionKey from internal AskUser requests', async () => {
    ...
    await expect(server.askUserInternal(
      makeQuestions(),
      1_000,
      { sessionKey: 'agent:main:egoai:session-b' },
    )).resolves.toEqual({ behavior: 'deny' });

    expect(received).toHaveLength(1);
    expect(received[0].sessionKey).toBe('agent:main:egoai:session-b');
  });
});
