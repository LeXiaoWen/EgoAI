...
vi.mock('./openclawLocalExtensions', () => ({
  ...
  hasBundledOpenClawExtension: (id: string) => (
    id !== 'qwen-portal-auth'
    && (id !== 'egoai-model-compat' || mockRuntimeState.modelCompatPluginAvailable)
  ),
...
describe('OpenClawConfigSync runtime config output', () => {
  ...
  test('writes OpenClaw config fields required by EgoAI patches', async () => {
    ...
    const result = sync.sync('egoai-patch-dependent-fields');
    expect(result.ok).toBe(true);
  ...
  test('activates deterministic Kimi K3 ownership for exact custom and package models', async () => {
    ...
    expect(customProvider.api).toBe('egoai-model-compat');
    expect(serverProvider.api).toBe('egoai-model-compat');
    ...
    expect(config.plugins.entries['egoai-model-compat']).toEqual({
      enabled: true,
      config: {
        modelProfiles: {
    ...
    expect(config.plugins.allow).toContain('egoai-model-compat');
      ...
      plugins: {
        entries: {
          'egoai-model-compat': {
            enabled: true,
            config: {
              modelProfiles: compatConfig.plugins.entries['egoai-model-compat'].config.modelProfiles,
              thinkingProfiles: {
  ...
  test('assigns mixed-provider compatibility ownership independently of model order', async () => {
    ...
    for (const providers of [forward, reverse]) {
      expect(providers.custom_0.api).toBe('egoai-model-compat');
  ...
  test('writes server thinking profiles without taking over the provider transport', async () => {
    ...
    mockRuntimeState.serverModels = [{
      ...
      requestCapabilities: ['egoai-options-v1'],
    }];
  ...
  test('keeps legacy thinking transport when the server does not advertise request options', async () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    expect(
      config.plugins.entries['egoai-model-compat']
  ...
  test('fails closed when the Kimi K3 compatibility extension is unavailable', async () => {
    ...
    expect(result.error).toContain('egoai-model-compat');
    expect(fs.existsSync(configPath)).toBe(false);
  });
  ...
  test('enables media generation plugin when media entitlement is available', async () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    expect(config.plugins.entries['ego-media-generation']).toEqual({
      enabled: true,
      config: {
  ...
  test('writes managed browser policy forcing host target', async () => {
    ...
    expect(agentsMd).toContain('EgoAI does not support sandbox browser execution in this version.');
...
describe('resolveModelSourceForOpenClawProvider', () => {
  ...
  test('classifies the EgoAI plan without any Settings entry', async () => {
