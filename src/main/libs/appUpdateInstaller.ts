...
export async function downloadUpdate(
  ...
  const ts = Date.now();
  const downloadPath = path.join(updateDir, `egoai-update-${source}-${ts}${ext}.download`);
  const finalPath = path.join(updateDir, `egoai-update-${source}-${ts}${ext}`);
...
async function installWindowsNsis(
  ...
  // customCheckAppRunning macro stops remaining EgoAI processes by image
  // name and polls until they are gone before replacing files. The installer
  // process itself is named egoai-update-*, so it is not affected by that
  // kill.
