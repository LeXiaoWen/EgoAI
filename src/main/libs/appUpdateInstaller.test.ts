...
const INSTALLER_PATH = 'C:\\Users\\test\\AppData\\Roaming\\EgoAI\\updates\\egoai-update-manual-1.exe';
...
describe('Windows update download URL enforcement', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-download-policy-'));
  ...
  test('rejects an insecure input before fetching or creating a partial file', async () => {
    await expect(downloadUpdate(
      'http://downloads.example.com/EgoAI.exe',
      'manual',
      () => {},
  ...
  test('downloads directly without relying on Electron response.url', async () => {
    ...
    const inputUrl =
      'https://downloads.example.com/EgoAI.exe?inputToken=do-not-log';
  ...
  test('leaves no cached file when the Windows fetch rejects a redirect', async () => {
    ...
    await expect(downloadUpdate(
      'https://downloads.example.com/EgoAI.exe',
      'auto',
      () => {},
...
describe('hdiutil plist parsing', () => {
  test('extracts the mount point and dev entries from an APFS attach result', () => {
    ...
    const json = JSON.stringify({
      'system-entities': [
        ...
        { 'dev-entry': '/dev/disk5s1', 'mount-point': '/Volumes/EgoAI', 'volume-kind': 'apfs' },
    ...
    expect(result.mountPoint).toBe('/Volumes/EgoAI');
  ...
  test('extracts the mount point from an HFS attach result', () => {
    const json = JSON.stringify({
      'system-entities': [
        { 'content-hint': 'GUID_partition_scheme', 'dev-entry': '/dev/disk4' },
        { 'content-hint': 'Apple_HFS', 'dev-entry': '/dev/disk4s1', 'mount-point': '/Volumes/EgoAI 1' },
      ],
    });

    const result = parseHdiutilAttachOutput(json);

    expect(result.mountPoint).toBe('/Volumes/EgoAI 1');
  });
...
describe('mac swap builders', () => {
  test('places staging and backup next to the target app, hidden and not .app-suffixed', () => {
    const swapPaths = buildMacSwapPaths('/Applications/Ego AI.app', 1234);

    expect(path.dirname(swapPaths.staging)).toBe('/Applications');
    expect(path.dirname(swapPaths.backup)).toBe('/Applications');
    expect(path.basename(swapPaths.staging)).toBe(`.Ego AI.app${MAC_SWAP_STAGING_INFIX}1234`);
    expect(path.basename(swapPaths.backup)).toBe(`.Ego AI.app${MAC_SWAP_BACKUP_INFIX}1234`);
  ...
  test('builds a staged-copy, guarded-backup, rollback and cleanup sequence', () => {
    const target = '/Applications/EgoAI.app';
    const swapPaths = buildMacSwapPaths(target, 7);

    const cmd = buildMacSwapInstallCommand('/Volumes/EgoAI/EgoAI.app', target, swapPaths);

    const cpIndex = cmd.indexOf(`cp -R '/Volumes/EgoAI/EgoAI.app' '${swapPaths.staging}'`);
  ...
  test('single-quotes paths containing spaces and quotes', () => {
    const target = `/Applications/It's "Ego".app`;
    const swapPaths = buildMacSwapPaths(target, 7);

    const cmd = buildMacSwapInstallCommand('/Volumes/src.app', target, swapPaths);

    expect(cmd).toContain(`'/Applications/It'\\''s "Ego".app'`);
  });
});
...
describe('macOS DMG install', () => {
  ...
  const USER_DATA = '/Users/test/Library/Application Support/EgoAI';
  const DMG_PATH = `${USER_DATA}/updates/egoai-update-auto-1.dmg`;
  const TARGET_APP = '/Applications/EgoAI.app';
  ...
  const respondMountedAtVolumes = () => attachMountedJson('/Volumes/EgoAI');
  ...
  test('mounts, swap-installs and relaunches on the happy path', async () => {
    ...
    expect(detachCommands[0]).toContain('/Volumes/EgoAI');
  ...
  test('cleans up leftover staging and backup directories before installing', async () => {
    attachResponders = [respondMountedAtVolumes];
    applicationsEntries = [
      `.EgoAI.app${MAC_SWAP_STAGING_INFIX}1`,
      `.EgoAI.app${MAC_SWAP_BACKUP_INFIX}2`,
      'EgoAI.app',
      'Other.app',
    ];
    ...
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.EgoAI.app${MAC_SWAP_STAGING_INFIX}1`,
      { recursive: true, force: true },
    );
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.EgoAI.app${MAC_SWAP_BACKUP_INFIX}2`,
      { recursive: true, force: true },
    );
