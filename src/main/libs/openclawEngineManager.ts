...
// config change) is trusted to complete before EgoAI resumes managing the
// process itself.
...
export function buildOpenClawCompileCacheEnv(compileCacheDir: string): NodeJS.ProcessEnv {
  return {
    ...
    // The cache is already configured by EgoAI. Prevent the packaged
...
export class OpenClawEngineManager extends EventEmitter {
   ...
   * active EgoAI must not kill/respawn the process: the gateway is between
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      // Disable Bonjour/mDNS LAN discovery advertising.  EgoAI is a
    ...
    // find the EgoAI-managed Python instead of the Windows Store stub.
  ...
  private ensureOpenClawWorkerShimsForBundle(runtimeRoot: string): void {
    try {
      ...
      if (result.protectedExisting.length > 0) {
        console.warn(
          `[OpenClaw] Skipped ${result.protectedExisting.length} worker shim(s) because existing files are not EgoAI shims.`,
        );
      }
    } catch (error) {
