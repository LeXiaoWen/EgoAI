...
export type ServerModelMetadata = {
  ...
  requestCapabilities?: EgoAIRequestCapability[];
...
export function updateServerModelMetadata(models: ServerModelMetadataInput[]): boolean {
  ...
  for (const model of models) {
    ...
    const thinkingConfig = runtimeMetadata.supportsThinking === true
      ...
      : undefined;
    const requestCapabilities = parseEgoAIRequestCapabilities(model.requestCapabilities);
...
function tryEgoaiServerFallback(modelId?: string): MatchedProvider | null {
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryEgoaiServerFallback(configuredModelId);
  ...
  if (preferredProviderName === ProviderName.EgoaiServer) {
    const serverMatch = tryEgoaiServerFallback(modelId);
    if (serverMatch) {
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-egoai-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'egoai-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    ...
    result[envName] = apiKey || 'sk-egoai-local';
  }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.EgoaiServer) continue;
