...
const makeTempWorkspace = (): string => fs.mkdtempSync(path.join(os.tmpdir(), 'ego-workspace-'));
...
test('workspace rehydration bridge includes bounded capsule and git state', async () => {
  ...
  expect(bridge).toContain('[EgoAI workspace state after context compaction]');
...
test('workspace rehydration bridge skips invalid workspace but keeps capsule state', async () => {
  const bridge = await buildCoworkWorkspaceRehydrationBridge({
    sessionId: 'session-1',
    cwd: path.join(os.tmpdir(), 'missing-ego-workspace'),
    capsule: makeCapsule(),
  });
