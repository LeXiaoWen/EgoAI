...
const makeTempDir = (): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-data-migration-test-'));
  tempRoots.push(dir);
  return dir;
};
...
const writeManifestFixture = (userDataPath: string): void => {
  writeFile(
    path.join(userDataPath, '.egoai-migration.json'),
    JSON.stringify({
      format: 'egoai-data-migration',
      version: 1,
      archiveRoot: 'EgoAI',
      sqlite: { exists: true },
...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, 'EgoAI');
  ...
  writeFile(path.join(userData, 'backups', 'sqlite', 'snapshots', 'egoai-latest.sqlite'), 'old-snapshot');
  writeFile(path.join(userData, 'sqlite-backups', 'egoai-latest.sqlite'), 'legacy-snapshot');
  ...
  expect(entries).toContain('EgoAI/.egoai-migration.json');
  expect(entries).toContain(`EgoAI/${DB_FILENAME}`);
  expect(entries).toContain('EgoAI/openclaw/state/openclaw.json');
  expect(entries).toContain('EgoAI/SKILLs/demo/SKILL.md');
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, 'EgoAI', '.egoai-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.format).toBe('egoai-data-migration');
  expect(manifest.archiveRoot).toBe('EgoAI');
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, 'EgoAI', DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, 'EgoAI', `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', 'EgoAI');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/does not contain EgoAI user data/);
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'EgoAI');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/missing \.egoai-migration\.json/);
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'EgoAI');
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, 'EgoAI', '.egoai-migration.json');
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'EgoAI');
  const targetUserData = path.join(root, 'target', 'EgoAI');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  writeFile(path.join(sourceUserData, 'backups', 'sqlite', 'snapshots', 'egoai-latest.sqlite'), 'source-backup');
  writeFile(path.join(sourceUserData, 'sqlite-backups', 'egoai-latest.sqlite'), 'source-legacy-backup');
  ...
  writeFile(path.join(targetUserData, 'old-only.txt'), 'old');
  writeFile(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'egoai-latest.sqlite'), 'target-backup');
  writeFile(path.join(targetUserData, 'sqlite-backups', 'egoai-latest.sqlite'), 'target-legacy-backup');
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, 'EgoAI', 'backups', 'sqlite', 'snapshots', 'egoai-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, 'EgoAI', 'sqlite-backups', 'egoai-latest.sqlite'), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, 'EgoAI', 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, 'EgoAI', 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, 'EgoAI', 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, 'EgoAI', 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, 'EgoAI', 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, 'EgoAI', 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, 'EgoAI', 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, 'EgoAI', 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, 'EgoAI', 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, 'EgoAI', 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, 'EgoAI', 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
  ...
  expect(fs.existsSync(path.join(targetUserData, `${DB_FILENAME}-shm`))).toBe(false);
  expect(fs.existsSync(path.join(targetUserData, 'old-only.txt'))).toBe(false);
  expect(fs.readFileSync(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'egoai-latest.sqlite'), 'utf8'))
    .toBe('target-backup');
  expect(fs.readFileSync(path.join(targetUserData, 'sqlite-backups', 'egoai-latest.sqlite'), 'utf8'))
