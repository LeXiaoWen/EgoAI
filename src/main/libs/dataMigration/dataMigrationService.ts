...
const MANIFEST_FILE_NAME = '.egoai-migration.json';
const PENDING_RESTORE_FILE_NAME = '.egoai-data-migration-restore-pending.json';
const LAST_RESTORE_RESULT_FILE_NAME = '.egoai-data-migration-restore-result.json';
const ARCHIVE_FORMAT = 'egoai-data-migration';
...
export const buildDataMigrationBackupFileName = (date = new Date()): string =>
  `egoai-backup-${formatDataMigrationTimestamp(date)}.tar.gz`;

export const buildDataMigrationRollbackFileName = (date = new Date()): string =>
  `egoai-rollback-${formatDataMigrationTimestamp(date)}.tar.gz`;
...
export const createMigrationArchiveSync = (
  ...
  const archiveKind = input.archiveKind ?? 'backup';
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-data-migration-'));
...
const validateArchiveContentSync = (archivePath: string, root: string): void => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-data-migration-inspect-'));
  try {
    tar.extract({
      sync: true,
      file: archivePath,
...
export const inspectMigrationArchiveSync = (
  ...
  if (!state.root || state.entryCount <= 0) {
    throw new Error('Backup archive is empty or missing EgoAI user data.');
  }
...
const extractMigrationArchiveToTempSync = (
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-data-migration-restore-'));
  ...
  try {
    ...
    if (!fs.existsSync(sourceRoot) || !fs.statSync(sourceRoot).isDirectory()) {
      throw new Error('Backup archive did not extract a valid EgoAI user data directory.');
    }
