...
test('refreshes EgoAI credentials for 401 but not 403', () => {
  expect(testUtils.shouldRefreshEgoAIToken(401)).toBe(true);
  expect(testUtils.shouldRefreshEgoAIToken(200)).toBe(false);
  expect(testUtils.shouldRefreshEgoAIToken(403)).toBe(false);
});
...
test('extracts EgoAI monthly quota error from proxy SSE packet', () => {
  const packet = [
    'event: error',
...
test('ignores generic HTTP 402 without EgoAI quota code or message', () => {
  const packet = [
    'event: error',
...
test('adds fixed capability, client version, and enterprise context headers without trusting incoming values', () => {
  expect(testUtils.buildUpstreamRequestHeaders(
    'access-token',
    {
      ...
      'x-egoai-client-capabilities': 'attacker-controlled',
      'x-egoai-client-version': '0.0.0',
    },
    '2026.7.23',
    {
