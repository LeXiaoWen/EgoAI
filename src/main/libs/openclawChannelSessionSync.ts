 ...
 *
 * Discovers sessions created by OpenClaw channel extensions (weixin, wecom, qq,
 * email) and maps them to local Cowork sessions so that IM conversations are
 * visible in the EgoAI UI.
 */
 ...
 *    where jsonObject carries the channel/account/peer fields.
...
export function parseChannelSessionKey(
  ...
  // For HTTP-originating sessions, the format is:
...
const MULTI_INSTANCE_PLATFORMS = new Set<Platform>(['qq', 'wecom', 'email']);
 ...
 *   - "{accountId}:{peerKind}:{peerId}"      e.g. "bot1:group:12345"
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip EgoAI-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: EgoAI-originated session');
      return null;
    }
   ...
   * (e.g. an outbound delivery that just went out to `channel` + `to`).
  ...
  /** Check whether a sessionKey belongs to a recognized channel or the main agent session. */
