...
function sanitizeShellGetBrowserAppsInput(input: unknown): ShellGetBrowserAppsInput {
  ...
  if (source.projectDirectory === undefined || source.projectDirectory === null) return {};
  if (typeof source.projectDirectory !== 'string') {
    throw new Error('projectDirectory must be a string.');
  }
  const projectDirectory = source.projectDirectory.trim();
  if (!projectDirectory) {
    throw new Error('projectDirectory is required.');
  }
  if (projectDirectory.length > 4096) {
    throw new Error('projectDirectory is too long.');
  }
...
const buildAvailableOpenClawProviders = (): Record<string, { models: Array<{ id: string }> }> => {
  ...
  if (serverModelIds.length > 0) {
    const serverProvider = providerMap[OpenClawProviderId.EgoaiServer]
      ?? { models: [] };
    ...
    providerMap[OpenClawProviderId.EgoaiServer] = serverProvider;
  }
...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'egoai', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `egoai-logs-${datePart}-${timePart}.zip`;
};
...
const _syncOpenClawConfigImpl = async (
  ...
  console.log(
    `${D()} sync() ok=${syncResult.ok} changed=${syncResult.changed} restartImpact=${syncResult.restartImpact ?? OpenClawConfigImpact.None}`,
  );
  if (!syncResult.ok) {
  ...
  const effectiveConfigChanged =
    syncResult.changed || pluginInstallMigrationChanged;
  console.log(
    `${D()} final config changed=${effectiveConfigChanged} (sync=${syncResult.changed} legacyMigration=${pluginInstallMigrationChanged})`,
  );
  ...
  // Force a hard restart when env changed, or when the caller explicitly
  // requires a running gateway restart.
  ...
  console.log(
    `${D()} needsHardRestart=${needsHardRestart} (envChanged=${secretEnvVarsChanged} configChanged=${effectiveConfigChanged} restartImpact=${syncResult.restartImpact ?? OpenClawConfigImpact.None} expectedRestart=${expectedRestartImpact} restartFlag=${!!options.restartGatewayIfRunning})`,
  );
...
function mergeCoworkSystemPrompt(systemPrompt?: string): string | undefined {
  ...
  return normalizedSystemPrompt || undefined;
}
...
if (!gotTheLock) {
  ...
  if (!app.isPackaged) {
    ...
    app.setAsDefaultProtocolClient('egoai', process.execPath, [
            ...
            ? [
                {
                  ...
                  filePath: path.join(app.getPath('appData'), 'EgoAI', 'install-timing.log'),
                },
              ]
            : []),
        ],
  ...
  ipcMain.handle(DataMigrationIpc.Backup, async event => {
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to back up EgoAI data',
      };
    }
  });
  ...
  ipcMain.handle(DataMigrationIpc.Restore, async event => {
    ...
    try {
      ...
      return {
        ...
        error: success ? undefined : restoreResult?.error || 'Failed to import EgoAI data backup',
      };
    } catch (error) {
      isCleanupInProgress = false;
      const message = error instanceof Error ? error.message : 'Failed to import EgoAI data backup';
  ...
  ipcMain.handle(ShellIpc.OpenHtmlInBrowser, async (_event, htmlContent: string) => {
    try {
      const tmpDir = path.join(os.tmpdir(), 'egoai-preview');
...
<body>
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'egoai', 'project');
