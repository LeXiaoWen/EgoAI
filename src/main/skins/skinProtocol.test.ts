...
function createManagedAsset(): {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egoai-skin-protocol-'));
...
describe('skin protocol URL', () => {
  ...
  test('rejects unsupported hosts, slots, traversal, credentials, and query parameters', () => {
    ...
    expect(parseSkinProtocolUrl('ego-skin://other/skin-one/workspace.backdrop')).toBeNull();
    expect(parseSkinProtocolUrl('ego-skin://asset/skin-one/unknown.slot')).toBeNull();
    expect(parseSkinProtocolUrl('ego-skin://asset/%2e%2e/workspace.backdrop')).toBeNull();
    expect(parseSkinProtocolUrl('ego-skin://asset/skin-one/workspace%2fbackdrop')).toBeNull();
    expect(parseSkinProtocolUrl('ego-skin://user@asset/skin-one/workspace.backdrop')).toBeNull();
    expect(parseSkinProtocolUrl('ego-skin://asset/skin-one/workspace.backdrop?path=local')).toBeNull();
  });
});
