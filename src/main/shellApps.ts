...
const BROWSER_APPS_PROBE_FILE = path.join(os.tmpdir(), 'egoai-browser-probe.html');
...
async function ensureBrowserProbeFile(): Promise<void> {
  try {
    await fs.promises.writeFile(
      ...
      '<!doctype html><meta charset="utf-8"><title>EgoAI browser probe</title>',
      'utf8',
    );
  } catch {
...
async function icnsToPng(icnsPath: string): Promise<string | null> {
  const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'egoai-app-icon-'));
