...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'EgoAI Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'EgoAI Skill Downloader',
...
export class SkillManager {
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            ...
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'egoai-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'egoai-skill-npm-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'egoai-skill-clawhub-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'egoai-skill-'));
        cleanupPath = tempRoot;
  ...
  private async performUpgradeDownload(skillId: string, downloadUrl: string, existingSkillDir: string): Promise<{
    ...
    try {
      ...
      const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'egoai-skill-upgrade-'));
      cleanupPath = tempRoot;
