...
describe('sessionDiagnosticsArchive', () => {
  test('builds a stable diagnostics zip file name', () => {
    ...
    expect(fileName).toBe(
      'egoai-diagnostics-LOFTER content safety review Prompt-session--20260703-010203.zip',
    );
  });
  ...
  test('normalizes unusual characters and bounds the file name length', () => {
    ...
    expect(fileName).toBe(
      `egoai-diagnostics-CON ${'会'.repeat(36)}-session--20260703-010203.zip`,
    );
    expect(Array.from(fileName)).toHaveLength(87);
