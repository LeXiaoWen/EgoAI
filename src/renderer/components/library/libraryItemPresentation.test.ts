...
describe('library item presentation', () => {
  test('uses the artifact title as the display file name for local artifacts', () => {
    expect(getLibraryDisplayFileName(makeLocalItem({ title: '员工信息表.xlsx' }))).toBe('员工信息表.xlsx');
  });
});
