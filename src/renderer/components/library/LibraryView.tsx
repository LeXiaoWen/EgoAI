...
}> = ({ item, viewMode, onOpen, menuItems }) => {
  const list = viewMode === LibraryViewMode.List;
  if (list) {
    return (
      <article
        data-library-item-key={item.itemId}
        role="button"
        tabIndex={0}
        onClick={onOpen}
        onKeyDown={event => {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 pt-2 pr-10">
        ...
        <div className={`mt-1 flex min-w-0 items-center gap-1.5 ${MANAGEMENT_META_TEXT} leading-[var(--ego-leading-xs)] text-secondary`}>
          <span className="truncate">{getLibrarySourceLabel()}</span>
          <span aria-hidden="true">·</span>
          <span className="truncate">{getLibraryItemStatus(item)}</span>
        </div>
        <div className={`${MANAGEMENT_META_TEXT} mt-1 leading-[var(--ego-leading-xs)] text-tertiary`}>
...
const LibraryViewContent: React.FC<LibraryViewProps> = ({
  ...
  const queryKey = `local:${localQueryKey}`;
  const hasResolvedSnapshot = localResolvedQueryKey.length > 0;
  const hasResolvedCurrentQuery = localResolvedQueryKey === localQueryKey;
  ...
  const loadData = useCallback(async (
    ...
    const loadLocalPage = !append || localData.hasMore;
  ...
  refreshBatchHandlerRef.current = async batch => {
    ...
    try {
      ...
      setActiveItem(current => (
        current ? (items.find(item => item.itemId === current.itemId) ?? current) : current
      ));
  ...
  useEffect(() => {
    refreshCoordinatorRef.current?.setActive(loadPhase === LibraryLoadPhase.Settled);
  }, [loadPhase]);
  ...
  const updateFavorite = async (item: LibraryItem): Promise<void> => {
    ...
    if (!result.success) {
      ...
      setActiveItem(current => current?.itemId === item.itemId ? { ...current, isFavorite: item.isFavorite } : current);
    }
  };
  ...
  const getCardActionLabel = (
    ...
    return i18nService.t('libraryRevealFile');
  };
  ...
  const buildCardMenuItems = (item: LibraryItem): CardOverflowMenuItem[] => (
    getLibraryCardActionIds().map(action => ({
      key: action,
  ...
  const hasMore = hasResolvedCurrentQuery && localData.hasMore;
          ...
          <div>
            {loading ? (
              ...
              <div className="mt-12 rounded-2xl border border-dashed border-border py-16 text-center">
                ...
                <p className={`${MANAGEMENT_BODY_TEXT} mt-1 leading-[var(--ego-leading-sm)] text-secondary`}>
                  {i18nService.t(hasActiveLocalFilter
