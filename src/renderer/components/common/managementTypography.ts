 ...
 *
 * Sizes derive from `--ego-ui-font-size` — the single "UI font size" the
 ...
 *
 * `calc()` rather than a `--ego-text-*` token: those are written through
...
export const MANAGEMENT_PAGE_TITLE_TEXT =
  'text-[length:calc(var(--ego-ui-font-size)_+_0.5px)]';
...
export const MANAGEMENT_BODY_TEXT =
  'text-[length:calc(var(--ego-ui-font-size)_-_1px)]';

/** Badge strips: card meta, counts, pills, stat labels. */
export const MANAGEMENT_META_TEXT =
  'text-[length:calc(var(--ego-ui-font-size)_-_3px)]';
