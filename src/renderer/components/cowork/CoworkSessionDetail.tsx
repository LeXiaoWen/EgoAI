...
): string => normalizeProjectDirectoryForDedup(projectDirectory || '');
...
const buildRailItems = (
  ...
  for (let index = 0; index < turns.length; index += 1) {
    ...
    items.push({
      ...
      label: turn.userMessage ? getRailLabel(userContent, `Turn ${index + 1}`) : 'EgoAI',
      summary: assistantContent
        ? getRailLabel(assistantContent, 'EgoAI', COWORK_RAIL_TOOLTIP_PREVIEW_MAX_LENGTH)
        : '',
...
const composeExportCanvas = async (
  ...
  ctx.fillText('EgoAI — 全场景办公助手 Agent', textX, footerCenterY - taglineFontSize / 2 - 2);

  ctx.fillStyle = subtitleColor;
  ctx.font = `400 ${taglineFontSize}px ${fontStack}`;
  ctx.fillText('通用桌面 Agent · 数据本地优先', textX, footerCenterY + brandFontSize / 2 + 3);
...
const CoworkSessionDetail: React.FC<CoworkSessionDetailProps> = ({
  ...
  const handleCompactContext = useCallback(() => {
    ...
    currentSession?.id,
    currentSession?.status,isContextBusy,
    isSessionBusy]);
  ...
  const getToolGroupSubagents = useCallback((group: ToolGroupItem): SubagentSessionSummary[] => {
    ...
    const candidateIds = [
      group.toolUse.id,
      group.toolUse.metadata?.toolUseId].filter((value): value is string => typeof value === 'string' && value.length > 0);
