...
const CoworkPromptInput = React.forwardRef<CoworkPromptInputRef, CoworkPromptInputProps>(
  ...
  const textareaClass = isCompact
    ? `w-full resize-none bg-transparent px-4 pb-1.5 text-sm leading-[var(--ego-leading-sm)] text-foreground placeholder:dark:text-foregroundSecondary/60 placeholder:text-secondary/60 focus:outline-none min-h-[${minHeight}px] max-h-[${maxHeight}px] ${hasActiveContext ? 'pt-1.5' : 'pt-2'}`
    : isLarge
    ? `w-full resize-none bg-transparent px-4 pb-2 text-foreground placeholder:dark:text-foregroundSecondary/60 placeholder:text-secondary/60 focus:outline-none min-h-[${minHeight}px] max-h-[${maxHeight}px] ${
      useHomeContextLayout
        ? `${hasActiveContext ? 'pt-2' : 'pt-3'} text-sm leading-[var(--ego-leading-prompt)]`
        : `${hasActiveContext ? 'pt-2' : 'pt-2.5'} text-[length:var(--ego-text-promptLarge)] leading-[var(--ego-leading-promptLarge)]`
    }`
  ...
  const renderMentionTextarea = ({
    ...
    placeholder: string;
        ...
        style={{
          ...style,
          caretColor: 'var(--ego-text-primary)',
          }}
        />
      </div>
    );
