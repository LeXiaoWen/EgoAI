...
const CoworkView: React.FC<CoworkViewProps> = ({
  ...
  const handleStartSession = async (
    ...
    try {
      ...
      logCoworkViewModel(
        `creating session with model ${sessionModelOverride || 'default'}; agent model is ${currentAgent?.model || 'empty'}`,
      );
          ...
          <div className="relative z-10 flex-1 overflow-y-auto min-h-0">
            <div className="relative flex min-h-full w-full min-w-[320px] flex-col items-center px-4 py-8">
              ...
              <div data-skin-home-copy="true" className="w-full max-w-3xl text-center">
                ...
                />
                <h2
                  className="mt-4 text-2xl font-semibold leading-[var(--ego-leading-2xl)] tracking-normal text-foreground animate-fade-in-up"
                  style={{ animationDelay: '70ms', animationFillMode: 'both' }}
                >
                  {i18nService.t(resolveHomeGreetingKey())}
                </h2>
                <p
                  className="mt-2 text-[length:var(--ego-text-promptLarge)] font-normal leading-[var(--ego-leading-promptLarge)] text-secondary animate-fade-in-up"
