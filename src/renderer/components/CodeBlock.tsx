...
const baseTheme = EditorView.theme({
  '&': {
    fontSize: 'var(--ego-code-font-size)',
  ...
  '.cm-lineNumbers .cm-gutterElement': {
    minWidth: '2.5em',
    padding: '0 8px 0 4px',
    fontSize: 'calc(var(--ego-code-font-size) - 1px)',
  },
  ...
  '.cm-search-custom': {
    ...
    gap: '4px',
    padding: '5px 10px',
    borderBottom: '1px solid var(--ego-border)',
    background: 'var(--ego-surface-raised)',
    fontFamily: "'SF Mono', 'Fira Code', Menlo, Monaco, 'Courier New', monospace",
    fontSize: 'var(--ego-code-font-size)',
  },
  '.cm-search-input': {
    ...
    color: 'var(--ego-foreground)',
    fontSize: 'var(--ego-code-font-size)',
    outline: 'none',
  },
  '.cm-search-input:focus': {
    borderColor: 'var(--ego-primary)',
  },
  '.cm-search-count': {
    flex: '0 0 auto',
    minWidth: '36px',
    fontSize: 'calc(var(--ego-code-font-size) - 1px)',
    color: 'var(--ego-text-secondary)',
    textAlign: 'center',
  ...
  '.cm-search-nav-btn:hover': {
    background: 'var(--ego-surface-hover)',
    color: 'var(--ego-foreground)',
  },
  '.cm-search-sep': {
    width: '1px',
    height: '16px',
    background: 'var(--ego-border)',
    margin: '0 2px',
    flex: '0 0 auto',
  },
  '.cm-search-opt': {
...
const darkThemeExt = EditorView.theme({
  ...
  '.cm-gutters': { backgroundColor: 'transparent', color: 'var(--ego-text-muted)' },
  '&.cm-focused .cm-selectionBackground, .cm-selectionBackground': {
    backgroundColor: 'color-mix(in srgb, var(--ego-primary) 30%, transparent) !important',
  },
...
const lightThemeExt = EditorView.theme({
  ...
  '.cm-gutters': { backgroundColor: 'transparent', color: 'var(--ego-text-muted)' },
  '&.cm-focused .cm-selectionBackground, .cm-selectionBackground': {
    backgroundColor: 'color-mix(in srgb, var(--ego-primary) 16%, transparent) !important',
  },
  '.cm-deletedChunk .cm-line, .cm-changedChunk .cm-deletedLine': {
  ...
  return createPortal(
    ...
    >
      ...
      <div
        ...
        style={{ flex: 1, minHeight: 0, backgroundColor: 'var(--ego-surface)' }}
  ...
  <button
    ...
    className={[
      ...
      active
        ...
        : 'text-secondary hover:bg-surface hover:text-foreground'].join(' ')}
...
const CodeBlock: React.FC<CodeBlockProps> = ({ node, className, children, ...props }) => {
  ...
  if (isInline) {
    const inlineClassName = [
      ...
      normalizedClassName].filter(Boolean)
      .join(' ');
    return (
