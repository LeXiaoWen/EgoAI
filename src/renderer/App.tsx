...
const App: React.FC = () => {
  ...
  const runInitPass = useCallback(async (mode: InitPassMode): Promise<void> => {
    ...
    try {
      ...
      // Privacy consent is an authorization gate, not optional startup data.
      // Resolve it before any degraded/default-config path can expose the
      // application shell. Retries issue a fresh IPC invoke to recover from an
      // early renderer/main handshake stall.
      if (!requiredStartupGatesReadyRef.current) {
        const privacyReady = await runStep('privacy check', async () => {
        ...
        if (!privacyReady) {
          throw new Error('Required startup gate unavailable (privacy).');
        }
        requiredStartupGatesReadyRef.current = true;
        mark('required privacy gate done');
      }
      ...
      if (!configReady) {
        ...
        // auth/listeners are never skipped.
