...
describe('sanitizeDshErrorDetail', () => {
  ...
  test('replaces a macOS home (with a spaced user name) by ~ and keeps the tail', () => {
    const error = new Error('Invalid dsh runtime manifest at /Users/jane doe/Library/Application Support/EgoAI/dsh/manifest.json');
    expect(sanitizeDshErrorDetail(error)).toBe(
      'Invalid dsh runtime manifest at ~/Library/Application Support/EgoAI/dsh/manifest.json'
    );
  });
  ...
  test('replaces a windows home with either separator', () => {
    expect(sanitizeDshErrorDetail(new Error('EPERM: C:\\Users\\Jane Doe\\AppData\\Roaming\\EgoAI\\dsh'))).toBe(
      'EPERM: ~\\AppData\\Roaming\\EgoAI\\dsh'
    );
