...
describe('parseLocalServiceUrlsFromText', () => {
  ...
  test('prefers project directory from startup cd command over session cwd', () => {
    const content = [
      '启动方式',
      '```bash',
      'cd /Users/admin/egoai/project/fanren/fanren-vote',
      'npm run dev',
      '```',
    ...
    const artifacts = parseLocalServiceUrlsFromText(
      ...
      'msg1',
      'sess1',
      { projectDirectory: '/Users/admin/egoai/project/fanren' },
    );

    expect(artifacts[0].localService?.projectDirectory).toBe(
      '/Users/admin/egoai/project/fanren/fanren-vote',
    );
  });

  test('prefers labeled project directory over session cwd', () => {
    const content = [
      '项目目录： `/Users/admin/egoai/project/fanren/fanren-vote/`',
      '预览地址：http://localhost:3000',
    ].join('\n');
    const artifacts = parseLocalServiceUrlsFromText(
      content,
  ...
  test('extracts labeled project directory from markdown file link', () => {
    const content = [
      ...
      '• 项目路径： [`/Users/admin/egoai/project/fanren/fanren-vote/`](file:///Users/admin/egoai/project/fanren/fanren-vote/)',
      '• 状态： 正常运行，HTTP 200',
    ].join('\n');
    const artifacts = parseLocalServiceUrlsFromText(
      content,
  ...
  test('extracts project directory from project located markdown file link', () => {
    const content = [
      '网站已全部完成！ 项目位于 [chinese-navy-site](file:///Users/admin/egoai/project/chinese-navy-site)，本地服务运行在 `http://localhost:8765`。',
      '',
      '**文件结构：**',
      '- [index.html](file:///Users/admin/egoai/project/chinese-navy-site/index.html) — 页面结构',
    ].join('\n');
    const artifacts = parseLocalServiceUrlsFromText(
      content,
      'msg1',
      'sess1',
      { projectDirectory: '/Users/admin/egoai/project' },
    );

    expect(artifacts[0].localService?.projectDirectory).toBe(
      '/Users/admin/egoai/project/chinese-navy-site',
    );
  });
  ...
  test('derives project directory from common parent of file links', () => {
    const content = [
      ...
      '- [index.html](file:///Users/admin/egoai/project/chinese-navy-site/index.html)',
      '- [style.css](file:///Users/admin/egoai/project/chinese-navy-site/styles/style.css)',
      '- [app.js](file:///Users/admin/egoai/project/chinese-navy-site/scripts/app.js)',
    ].join('\n');
    const artifacts = parseLocalServiceUrlsFromText(content, 'msg1', 'sess1');
...
describe('parseMediaTokensFromText', () => {
  ...
  test('parses macOS path with spaces (Application Support)', () => {
    const content = 'MEDIA: /Users/test/Library/Application Support/com.egoai.app/images/output.png';
    const artifacts = parseMediaTokensFromText(content, 'msg1', 'sess1');
    expect(artifacts).toHaveLength(1);
    expect(artifacts[0].filePath).toBe('/Users/test/Library/Application Support/com.egoai.app/images/output.png');
    expect(artifacts[0].type).toBe('image');
  });
