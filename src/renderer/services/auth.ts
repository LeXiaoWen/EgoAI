...
class AuthService {
  ...
  async handleCallback(code: string): Promise<boolean> {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        store.dispatch(clearMediaAccountState());
        this.applyAuthenticatedState(result.user, result.quota);
  ...
  private async performQuotaCheck(
    ...
    try {
      ...
      writeAuthRendererLog('debug', 'quota check completed');
      return { success: true };
    } catch (error) {
  ...
  async fetchProfileSummary() {
    ...
    try {
      ...
      const currentAuthState = store.getState().auth;
      if (!isAuthAccountRequestCurrent(authStateAtStart, currentAuthState)) {
