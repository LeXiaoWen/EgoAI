...
import {
  ...
  type ProjectDirectoryCandidate,
  ProjectDirectoryCandidateSource,
} from './artifactParser';
...
interface DirectoryEvidence {
  ...
  order: number;
  candidates: ProjectDirectoryCandidate[];
  ports: Set<number>;
...
function createToolDirectoryCandidate(
  directory: string,
  source: ProjectDirectoryCandidate['source'],
  confidence: number,
  messageId: string,
  ...
  const parsed = collectProjectDirectoryCandidatesFromText(
    ...
    messageId,
  ).find(candidate => candidate.source === ProjectDirectoryCandidateSource.TextLabeledPath);
...
function mergeCandidates(
  ...candidateGroups: Array<readonly ProjectDirectoryCandidate[] | undefined>
): ProjectDirectoryCandidate[] {
  const result: ProjectDirectoryCandidate[] = [];
...
function getShellDirectoryEvidence(
  ...
  const cdCandidates: ProjectDirectoryCandidate[] = [];
  ...
  for (const command of commands) {
    const commandCandidates = collectProjectDirectoryCandidatesFromText(
      ...
      fallbackDirectory,
      message.id,
    ).filter(candidate => candidate.source === ProjectDirectoryCandidateSource.TextCdCommand);
    cdCandidates.push(...commandCandidates);
  }

  const orderedCdCandidates = cdCandidates.reverse().map((candidate, index) => ({
    ...candidate,
    source: ProjectDirectoryCandidateSource.ToolCdCommand,
  ...
  const cwdCandidates = workingDirectories
    ...
    .map((directory, index) => createToolDirectoryCandidate(
      directory,
      ProjectDirectoryCandidateSource.ToolWorkingDirectory,
    ...
    .filter((candidate): candidate is ProjectDirectoryCandidate => Boolean(candidate));
...
function extractPwdResultCandidate(
  ...
  for (const rawLine of resultText.split(/\r?\n/)) {
    ...
    const candidate = createToolDirectoryCandidate(
      line,
      ProjectDirectoryCandidateSource.ToolPwdResult,
      95,
      message.id,
      workingDirectory,
...
function getClosestExplicitToolEvidence(
  ...
  for (let index = evidence.length - 1; index >= 0; index -= 1) {
    ...
    if (item.candidates.some(candidate =>
      candidate.source === ProjectDirectoryCandidateSource.ToolCdCommand ||
      candidate.source === ProjectDirectoryCandidateSource.ToolPwdResult
    )) {
      return item;
    }
  }
