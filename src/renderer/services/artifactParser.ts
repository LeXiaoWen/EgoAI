...
export const ProjectDirectoryCandidateSource = {
...
export type ProjectDirectoryCandidateSource =
  typeof ProjectDirectoryCandidateSource[keyof typeof ProjectDirectoryCandidateSource];

export interface ProjectDirectoryCandidate {
  directory: string;
  source: ProjectDirectoryCandidateSource;
  confidence: number;
  reason?: string;
  evidence?: string;
  messageId?: string;
...
function addProjectDirectoryCandidate(
  ...
  const candidate: ProjectDirectoryCandidate = {
    directory,
    source,
    confidence,
...
function selectBestProjectDirectoryCandidate(
  candidates: ProjectDirectoryCandidate[],
): ProjectDirectoryCandidate | undefined {
...
export function collectProjectDirectoryCandidatesFromText(
  ...
  while ((labelMatch = labelRe.exec(messageContent)) !== null) {
    addProjectDirectoryCandidate(
      ...
      ProjectDirectoryCandidateSource.TextLabeledPath,
      85,
      {
  ...
  while ((cdMatch = cdRe.exec(messageContent)) !== null) {
    addProjectDirectoryCandidate(
      ...
      ProjectDirectoryCandidateSource.TextCdCommand,
      80,
      {
        fallbackProjectDirectory,
  ...
  while ((fileLinkMatch = fileLinkRe.exec(messageContent)) !== null) {
    ...
    addProjectDirectoryCandidate(
      candidates,
      directory,
      ProjectDirectoryCandidateSource.TextFileLink,
      82,
      {
  ...
  if (commonDirectory) {
    addProjectDirectoryCandidate(
      candidates,
      commonDirectory,
      ProjectDirectoryCandidateSource.TextCommonParent,
      84,
      {
