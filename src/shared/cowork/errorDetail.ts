/**
 * Where the failing model comes from in EgoAI terms, so users can tell at
 * a glance whether an error concerns the EgoAI plan, a vendor coding plan
 * they configured, or their own custom provider.
 */
export const CoworkErrorModelSource = {
  EgoAIPlan: 'lobsterai-plan',
...
export interface CoworkErrorDetail {
  /** Runtime error message before EgoAI i18n normalization. */
  rawErrorMessage?: string;
  provider?: string;
  model?: string;
  /** EgoAI-side classification of where the failing model comes from. */
