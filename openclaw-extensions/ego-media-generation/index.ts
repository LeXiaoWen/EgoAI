...
import { isEgoAiDesktopSessionKey } from './sessionKey';
import {
  ...
  try {
    const response = await fetch(config.callbackUrl, {
      ...
      headers: {
        ...
        'x-ego-media-secret': config.secret,
      },
...
async function executeMediaStatusPolling(options: {
  ...
  return pollMediaStatus({
    ...
    log: message => options.log(`[ego-media-generation] ${message}`),
...
const plugin = {
  id: 'ego-media-generation',
  name: 'EgoMediaGeneration',
  description: 'Image/video generation and AI skin management tools powered by EgoAI.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    if (!config.callbackUrl || !config.secret) {
      api.logger.info('[ego-media-generation] skipped: callbackUrl or secret not configured.');
      return;
    }

    api.registerTool((ctx) => {
      const sessionKey = ctx.sessionKey ?? '';
      if (!isEgoAiDesktopSessionKey(sessionKey)) {
        return null;
      }
      ...
      return {
        ...
        description: [
          'Generate images using EgoAI server.',
          'Supports text-to-image and image-to-image generation.',
          'If the system prompt includes a EgoAI media reference mapping, use mapped file paths or URLs in image/images arguments and never pass @ media tokens as tool argument values.',
        ...
        async execute(
          ...
          if (action === MediaToolAction.Status) {
            ...
            try {
              ...
              api.logger.info(`[ego-media-generation] image status failed: toolCallId=${id} error=${message}`);
              return { content: [{ type: 'text', text: `Image status check failed: ${message}` }], isError: true };
            }
          }

          try {
            api.logger.info(`[ego-media-generation] image tool (${action}) started: toolCallId=${id} args=${JSON.stringify(sanitizeArgsForLog(args))}`);
            ...
            api.logger.info(`[ego-media-generation] image tool (${action}) completed: toolCallId=${id} elapsedMs=${Date.now() - startedAt} isError=${result.isError === true}`);
            return result;
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            api.logger.info(`[ego-media-generation] image tool callback failed: toolCallId=${id} error=${message}`);
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          'Generate videos using EgoAI server.',
          ...
          'If the system prompt includes a EgoAI media reference mapping, use mapped file paths or URLs in image/images/firstFrame/referenceImages/video/videos/media arguments and never pass @ media tokens as tool argument values.',
        ...
        async execute(
          ...
          if (action === MediaToolAction.Status) {
            ...
            try {
              ...
              const message = error instanceof Error ? error.message : String(error);
              api.logger.info(`[ego-media-generation] video status failed: toolCallId=${id} error=${message}`);
          ...
          try {
            api.logger.info(`[ego-media-generation] video tool (${action}) started: toolCallId=${id} args=${JSON.stringify(sanitizeArgsForLog(args))}`);
            const startedAt = Date.now();
            const result = await callMediaBridge(config, {
              tool: MediaToolName.VideoGenerate,
              args,
            ...
            api.logger.info(`[ego-media-generation] video tool (${action}) completed: toolCallId=${id} elapsedMs=${Date.now() - startedAt} isError=${result.isError === true}`);
            return result;
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            api.logger.info(`[ego-media-generation] video tool (${action}) failed: toolCallId=${id} error=${message}`);
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          'Create and manage a EgoAI AI skin pack through the trusted desktop callback.',
          ...
          'EgoAI deterministically infers a preferred light or dark appearance from presentation colors and applies it through the existing theme system; do not choose a color theme ID.',
        ...
        async execute(id: string, params: unknown) {
          ...
          try {
            api.logger.info(`[ego-media-generation] skin tool (${action}) started: toolCallId=${id} args=${JSON.stringify(sanitizeSkinArgsForLog(args))}`);
            const startedAt = Date.now();
            const result = await callMediaBridge(config, {
            ...
            api.logger.info(`[ego-media-generation] skin tool (${action}) completed: toolCallId=${id} elapsedMs=${Date.now() - startedAt} isError=${result.isError === true}`);
            return result;
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            api.logger.info(`[ego-media-generation] skin tool (${action}) failed: toolCallId=${id} error=${message}`);
    ...
    api.logger.info('[ego-media-generation] registered lobsterai_image_generate, lobsterai_video_generate, and lobsterai_skin_manage tools.');
  },
};
